...
export const getUpcomingEvents = async (): Promise<IUpEventData[]> => {
    try {
        const items = await sp.web.lists.getByTitle('NewsHomePage').items.getAll();
        return items.map(item => ({
            Id: item.Id,
            News: item.EventsDesc,
            ImgUrl: item.ImgUrl,
        }));
    } catch (error) {
...
export const postUpcomingEvent = async (event: IUpEventData): Promise<void> => {
    try {
        await sp.web.lists.getByTitle('NewsHomePage').items.add({
            EventsDesc: event.News,
            ImgUrl: event.ImgUrl,
        });
    } catch (error) {
...
export const deleteUpcomingEvent = async (eventId: number): Promise<void> => {
    try {
        await sp.web.lists.getByTitle('NewsHomePage').items.getById(eventId).delete();
    } catch (error) {
...
export const updateUpcomingEvent = async (event: IUpEventData): Promise<void> => {
    try {
        if (event.Id) {
            await sp.web.lists.getByTitle('NewsHomePage').items.getById(event.Id).update({
                EventsDesc: event.News,
                ImgUrl: event.ImgUrl,
            });
        } else {
